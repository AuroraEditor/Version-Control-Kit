...
public struct RevList {
    ...
    public func revRange(from: String, to: String) -> String {
        return "\(from)..\(to)"
    }
    ...
    public func getAheadBehind(directoryURL: URL,
        ...
        let result = try GitShell().git(args: args,
                                        path: directoryURL,
                                        name: #function,
                                        options: IGitExecutionOptions(expectedErrors: Set([GitError.BadRevision])))
        ...
        let pieces = result.stdout.components(separatedBy: .newlines)

        guard pieces.count == 2,
              let ahead = Int(pieces[0]),
              let behind = Int(pieces[1]) else {
            return nil
        }
    ...
    public func getCommitsBetweenCommits(directoryURL: URL,
                                         ...
                                         targetBranchSha: String) throws -> [Commit]? {
    ...
    public func getCommitsInRange(directoryURL: URL,
        ...
        for line in lines {
            ...
            if let match = commitSummaryRe.firstMatch(in: line, options: [], range: nsRange) {
                let sha = line.substring(with: match.range(at: 1))
                let summary = line.substring(with: match.range(at: 2))

                commits.append(Commit(sha: sha, summary: summary))
            }
        }
