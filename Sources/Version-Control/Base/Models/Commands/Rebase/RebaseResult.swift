...
enum RebaseResult {
    ...
    /// Git completed the rebase without reporting any errors, but the branch was already up to date and there was nothing to do.
    case alreadyUpToDate

    /// The rebase encountered conflicts while attempting to rebase, and these need to be resolved by the user before the rebase can continue.
    ...
    /// The rebase was not attempted because it could not check the status of the repository. The caller needs to confirm the repository is in a usable state.
    case aborted
