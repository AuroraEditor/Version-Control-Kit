...
///
/// This function generates a patch representing the changes introduced by a range of commits between the `base` and `head` references.
///
/// - Parameters:
...
///
/// - Throws: An error if there is a problem executing the `git format-patch` command or if the Git repository is not in a valid state.
///
/// - Example:
///   ```swift
...
///   ```
///
/// - Note: Ensure that you have the necessary permissions to execute Git commands in the specified directory, and that the Git repository is in a valid state.
public func formatPatch(directoryURL: URL, base: String, head: String) throws -> String {
    let result = try ShellClient.live().run(
        "cd \(directoryURL.relativePath.escapedWhiteSpaces());git format-patch --unified=1 --minimal --stdout \(base)..<\(head)"
    )
