...
public struct Reset {
    ...
    ///
    /// Use this function to convert a specified reset mode (e.g., hard, mixed, soft) and a reference to an array of Git command arguments that can be used for performing a reset operation.
    ///
    /// - Parameters:
    ///   - mode: The reset mode, represented by the `GitResetMode` enum.
    ///   - ref: A string that resolves to a reference, such as 'HEAD' or a commit SHA, to which the reset operation should be applied.
    ///
    ...
    ///
    /// - Note:
    ///   - This function is used to generate the Git command arguments for a reset operation based on the specified reset mode and reference.
    ...
    ///
    /// Use this function to reset the repository's HEAD and index to match a specific reference (e.g., commit or branch) using the specified reset mode.
    ///
    /// - Parameters:
    ...
    ///   - ref: A string that resolves to a reference, such as 'HEAD' or a commit SHA, to which the repository's HEAD and index should be reset.
    ///
    /// - Throws: An error if the operation fails. Possible errors include Git-related errors or issues with the provided repository path.
    ///
    ...
    ///
    /// - Warning:
    ///   Ensure that the provided `directoryURL` points to a valid Git repository directory. If the operation fails, check for potential issues with the repository or file permissions.
    ...
    ///   - paths: An array of strings representing the paths that should be updated in the index with information from the specified tree or commit.
    ///
    /// - Throws: An error if the operation fails. Possible errors include Git-related errors or issues with the provided repository path.
    ///
    /// - Example:
    ///   ```swift
    ///   let repositoryURL = URL(fileURLWithPath: "/path/to/your/repository") // Replace with the repository's directory URL
    ///   let pathsToUpdate = ["file1.txt", "folder/file2.txt"] // Replace with the paths you want to update
    ///
    ///   do {
    ...
    ///
    /// - Note:
    ///   - The `mode` parameter determines how the index will be reset (e.g., soft, mixed, hard). Refer to the `GitResetMode` enum for more details.
    ///   - This function updates the index to match the specified tree or commit, effectively altering the staging area's content.
    ///
    /// - Warning:
    ///   Ensure that the provided `directoryURL` points to a valid Git repository directory. If the operation fails, check for potential issues with the repository or file permissions.
    ///
    ...
    ///
    /// This function removes all changes from the staging area (index) of the Git repository, effectively "unstaging" them. Any changes that were previously staged but not committed will be reset.
    ///
    ...
    ///
    /// - Note:
    ///   This function is useful for undoing the staging of changes that were previously marked for commit but not yet committed. After calling this function, the changes will remain in the working directory but will no longer be part of the next commit.
    ///
    /// - Warning:
    ///   Ensure that the provided `directoryURL` points to a valid Git repository directory. If the operation fails, check for potential issues with the repository or file permissions.
    ///
