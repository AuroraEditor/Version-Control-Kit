...
/**
 This class provides methods to find and extract regex matches and their captured groups
 from a given text using NSRegularExpression.
 */
class Regex {
    ...
    /**
     ...
     - Returns: An array of arrays of strings representing the captured groups from each match. \
                The outer array contains one element for each match, and the inner arrays contain the captured strings.
     */
    ...
    /**
     ...
     - Returns: An array of NSTextCheckingResult objects representing the matched ranges in the text.\
       Each result corresponds to a single match found in the text.
     */
